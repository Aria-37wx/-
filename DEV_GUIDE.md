...
Step 4: db/models.py   — dataclass 定义        ?
...
- 查询意图无需确认按钮，AI 分析后直接展示结果（只读操作无风险）

### 对话流程

```
用户自然语言 → classify_intent() → 意图+参数 → 展示分析结果 → 用户确认 → 执行操作
         │
