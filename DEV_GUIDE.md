...
AI 对话具备多轮上下文记忆，能在同一话题上连续协作：

- 每轮查询/出库完成后，把「累积需求 requirement + 召回关键词 keyword + 已筛选结果 id 列表」写入 `ai_context`（会话状态）。
- 用户接下来的消息会连同上下文一起送入意图分类器，分类器返回两个额外布尔字段：
  - `refine=true`：用户是在**细化**上一轮查询（如"要强光的""用于照明的"）→ 累积需求后重新筛选，而不是当作全新查询。
  - `from_context=true`：用户是想把**上一轮筛选出的结果直接出库**（如"出库""借这些"）→ 直接用 `ai_context.result_ids` 拉取物料，不再重新搜索。
- 当用户开启一个**全新话题**（新的 search/outbound/inbound/recommend/return）时自动重置上下文；`unknown` 不重置（便于继续细化/出库）。

```
用户: "有没有发光元件"     → search → 召回 + 语义筛选 → 展示结果（记住上下文）
用户: "要强光的，用于照明"  → refine → 在上一轮结果上继续筛选
用户: "出库"               → from_context → 直接用筛选出的结果进入批量出库
```

> 离线规则（`classify_intent_fake`）通过 `_is_bare_outbound`（裸出库指令）和 `_looks_like_refinement`（约束/属性描述）
> 判断这两类续接；LLM 版则在 system prompt 中追加上下文说明让模型自行判断。概念级理解（如"发光元件"→ LED/OLED）依赖 LLM 关键词扩展。
