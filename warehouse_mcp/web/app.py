...
# ==================== AI 对话 ====================
    ...
    st.header("AI 助手")
    st.caption("用自然语言和我对话，我会自动判断你要入库、查询还是出库。")
    ...
    if st.session_state.ai_step == "showing_result" and st.session_state.ai_analysis:
        ...
        with st.chat_message("assistant"):
            if intent == "unknown":
                ...
                intent_labels = {"inbound": "入库", "outbound": "出库", "search": "查询", "return": "归还"}
...
# ==================== 出库 ====================
    ...
    if st.session_state.get("checkout_step") == "confirm":
        ...
        if c1.button("确认", type="primary"):
            result = checkout(d["id"], d["phone"], d["mode"])
            if "成功" in result:
...
# ==================== 归还 ====================
    ...
    if st.session_state.get("return_step") == "confirm":
        ...
        if phone.strip():
            ...
            try:
                rows = conn.execute(
                    """SELECT br.id, br.material_id, m.name, br.borrowed_at
            ...
            if not rows:
                ...
                for row in rows:
                    ...
                    with c1:
                        st.write(f"**{mat_name}**  |  归还编号 `{row['id']}`  |  {row['borrowed_at']}")
                    with c2:
